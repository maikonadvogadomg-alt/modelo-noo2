...
export async function sendMessage(
  ...
  if (!provider) throw new Error('Provedor nÃ£o encontrado. Configure nas ConfiguraÃ§Ãµes.');
  if (!config.apiKey) throw new Error('Chave de API nÃ£o configurada. VÃ¡ em ConfiguraÃ§Ãµes.');
  ...
  if (provider.type === 'anthropic') {
    const response = await fetch(`${provider.baseUrl}/messages`, {
      ...
      body: JSON.stringify({
        model: config.model,
        max_tokens: maxTok,
        system: systemPrompt ?? 'VocÃª Ã© um assistente inteligente e prestativo.',
...
export async function transcribeAudio(audioUri: string, apiKey: string): Promise<string> {
  ...
  if (!response.ok) throw new Error('Erro ao transcrever Ã¡udio');
  const data = await response.json();
