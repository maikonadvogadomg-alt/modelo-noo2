...
export async function importZip(): Promise<VirtualFile[]> {
  ...
  try {
            ...
            .catch(() => {
              zipEntry.async('base64').then((b64) => {
                files.push({
                  name: relativePath.split('/').pop() ?? relativePath,
                  path: relativePath,
                  content: `[arquivo binÃ¡rio â base64]\n${b64.slice(0, 200)}...`,
                });
              });
            }),
        );
    ...
    Alert.alert('Erro', `NÃ£o foi possÃ­vel ler o arquivo ZIP.\n${err instanceof Error ? err.message : ''}`);
    return [];
  }
}
...
export async function githubPullRepo(cfg: GitHubConfig): Promise<VirtualFile[]> {
  ...
  if (!branchRes.ok) throw new Error(`Branch "${cfg.branch}" nÃ£o encontrada`);
  const branchData = await branchRes.json();
  const treeSha = branchData.commit?.commit?.tree?.sha;
  if (!treeSha) throw new Error('Ãrvore do repositÃ³rio nÃ£o encontrada');
