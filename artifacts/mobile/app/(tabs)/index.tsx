...
const CODE_SYSTEM =
  'VocÃª Ã© um assistente especialista em programaÃ§Ã£o. Ajude a criar, debugar, revisar e melhorar cÃ³digo. Seja preciso, explique seu raciocÃ­nio e forneÃ§a cÃ³digo funcional completo. Use portuguÃªs do Brasil.';
...
export default function CodeHubScreen() {
  ...
  const [terminalLines, setTerminalLines] = useState<Array<{ type: 'cmd' | 'out' | 'err'; text: string }>>([
    { type: 'out', text: 'ââââââââââââââââââââââââââââââââââââââââ' },
    { type: 'out', text: 'â     SK Code Terminal  v2.0           â' },
    { type: 'out', text: 'ââââââââââââââââââââââââââââââââââââââââ' },
    { type: 'out', text: 'Digite "help" para ver os comandos disponÃ­veis' },
  ]);
    ...
    setTimeout(() => {
      switch (command) {
        case 'help':
          ...
          out('   SK Code Terminal â Comandos');
          out('  ââââââââââââââââââââââââââââââââââââââââââ');
          out('');
          out('  ââ Projeto (local) âââââââââââââââââââââââ');
          out('  ls              lista arquivos do projeto');
          out('  cat <arquivo>   exibe conteÃºdo de um arquivo');
          ...
          out('  git status      status do repositÃ³rio');
          out('  git log         histÃ³rico de commits');
        ...
        case 'cat': {
          ...
          if (!file) { err(`Arquivo nÃ£o encontrado: ${target}`); break; }
          out(`\n  ââ ${file.path} ââ`);
        ...
        case 'grep': {
          ...
          out(found > 0 ? `\n  ${found} ocorrÃªncia(s) encontrada(s)\n` : `\n  Nenhuma ocorrÃªncia de "${term}"\n`);
          break;
        }
        case 'stat': {
        ...
        case 'touch': {
          ...
          if (files.find((f) => f.path === name)) { err(`Arquivo jÃ¡ existe: ${name}`); break; }
        ...
        case 'rm': {
          ...
          if (!f) { err(`NÃ£o encontrado: ${name}`); break; }
        ...
        default: {
          ...
          fetch(`${apiBase}/api/terminal/exec`, {
            ...
            .then((data: { stdout: string; stderr: string; exitCode: number; cwd?: string }) => {
              ...
              if (data.exitCode === 0) {
                out(`  â ConcluÃ­do (exit 0)`);
              } else {
                err(`  â Exit code: ${data.exitCode}`);
              }
              setTimeout(() => terminalScrollRef.current?.scrollToEnd({ animated: true }), 100);
            })
            .catch((e: unknown) => {
              err(`  Erro de conexÃ£o: ${e instanceof Error ? e.message : 'Servidor indisponÃ­vel'}`);
              setTimeout(() => terminalScrollRef.current?.scrollToEnd({ animated: true }), 100);
            });
          break;
        }
  ...
  const promptNewFileAndroid = () => {
    ...
    Alert.alert('Novo Arquivo', 'Um arquivo vazio serÃ¡ criado.', [
    ...
    Alert.alert(file.name, 'O que deseja fazer?', [
      {
        ...
        onPress: () => {
          if (Platform.OS === 'ios') {
            ...
            Alert.alert('Renomear', 'Use a versÃ£o iOS ou edite via terminal (mv).');
          }
        },
      },
      {
  ...
  const githubPull = async () => {
    if (!githubConfig.token || !githubConfig.owner || !githubConfig.repo) {
      Alert.alert('GitHub', 'Configure o token, owner e repositÃ³rio em ConfiguraÃ§Ãµes.');
      return;
    }
    setGhLoading(true);
    setGhStatus('Baixando repositÃ³rio...');
    try {
      ...
      setGhStatus(`â ${pulled.length} arquivo(s) baixados`);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e: unknown) {
      setGhStatus(`â ${e instanceof Error ? e.message : 'Falhou'}`);
    } finally {
      setGhLoading(false);
      ...
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            setGhStatus(`â ${files.length} arquivo(s) enviados`);
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          } catch (e: unknown) {
            setGhStatus(`â ${e instanceof Error ? e.message : 'Falhou'}`);
          } finally {
        ...
        <View style={[phStyles.header, { paddingTop: topPad }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <View style={[phStyles.pill, { borderColor: '#e3b341' }]}>
              ...
              <Text style={[phStyles.pillText, { color: '#e3b341' }]}>JurÃ­dico</Text>
            </View>
            ...
            filteredProjects.map((p, i) => (
              <TouchableOpacity key={i} style={phStyles.projectCard} onPress={handleImportZip}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={phStyles.projectCardMeta}>{p.fileCount} arquivos Â· {p.date}</Text>
                </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: '#0d1117' }]}>
      ...
      {/* ââ Barra de tÃ­tulo estilo IDE ââ */}
      ...
      {ghStatus !== '' && (
        <View style={[styles.statusNotice, { backgroundColor: ghStatus.startsWith('â') ? '#1a2e1a' : ghStatus.startsWith('â') ? '#2e1a1a' : '#1a1f2e' }]}>
          <Text style={{ fontSize: 12, color: ghStatus.startsWith('â') ? '#3fb950' : ghStatus.startsWith('â') ? '#f85149' : '#79c0ff', fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace' }}>
            {ghStatus}
          </Text>
        </View>
      )}
      ...
      {!isConfigured && (
        <View style={styles.alertBar}>
          ...
          <Text style={styles.alertText}>Nenhuma chave de API configurada â vÃ¡ em ConfiguraÃ§Ãµes</Text>
        </View>
      )}

      {/* ââ Abas abertas ââ */}
      {openTabs.length > 0 && (
        <ScrollView
          horizontal
          ...
          <View style={{ flex: 1 }}>
            {activeTab ? (
              isEditing ? (
                <View style={{ flex: 1 }}>
                  <View style={styles.editorToolbar}>
                    ...
                    <Text style={styles.toolbarFileName} numberOfLines={1}>â {activeTab.name}</Text>
              ...
              <View style={styles.welcome}>
                ...
                <Text style={styles.welcomeSub}>IDE portÃ¡til com IA integrada</Text>
      ...
      {/* ââ FAB Terminal (canto inferior esquerdo) ââ */}
      <TouchableOpacity
      ...
      {/* ââ Modal Preview (tela cheia) ââ */}
      ...
      {/* ââ Modal Terminal (tela cheia) ââ */}
      <Modal visible={terminalOpen} animationType="slide" presentationStyle="fullScreen">
        <View style={styles.termModal}>
          ...
          <View style={[styles.termModalHeader, { paddingTop: Platform.OS === 'web' ? 20 : insets.top + 8 }]}>
            ...
            <Text style={styles.termModalTitle}>Terminal â {projectName}</Text>
      ...
      <View style={[styles.statusBar, { paddingBottom: Platform.OS === 'web' ? 4 : 0 }]}>
        {activeTab ? (
          <>
            ...
            <Text style={styles.statusSep}>Â·</Text>
        ...
        <View style={[styles.modal, { backgroundColor: '#0d1117' }]}>
          <View style={styles.modalHeader}>
            ...
            <View style={{ flex: 1, marginHorizontal: 12 }}>
              <Text style={styles.modalTitle}>Assistente de CÃ³digo</Text>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.chatEmpty}>
                ...
                <Text style={styles.chatEmptyTitle}>Assistente de CÃ³digo</Text>
                <Text style={styles.chatEmptyText}>
                  {activeTab
                    ? `Arquivo: ${activeTab.name}\nPergunte sobre o cÃ³digo, peÃ§a melhorias ou debug`
                    : 'Selecione um arquivo ou faÃ§a uma pergunta sobre programaÃ§Ã£o'}
                </Text>
              </View>
            }
          />
